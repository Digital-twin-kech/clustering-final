...
# Stage 1 Spatial Fixed: Split LAZ into Spatially Coherent Chunks
# Uses PDAL --summary to get exact bounds and create spatial chunks
# Usage: ./stage1_spatial_fixed.sh
...
# For ~5 chunks, use a 2x3 or 3x2 grid
GRID_X=3
GRID_Y=2
...
for ((x=0; x<GRID_X; x++)); do
    for ((y=0; y<GRID_Y; y++)); do
        ...
        if pdal translate "$INPUT_FILE" "$OUTPUT_CHUNK" \
           --filters.crop.bounds="([$CELL_MIN_X,$CELL_MAX_X],[$CELL_MIN_Y,$CELL_MAX_Y])" \
           --writers.las.compression=true 2>/dev/null; then

            if [[ -f "$OUTPUT_CHUNK" ]]; then
                # Get chunk info
                CHUNK_SIZE_MB=$(ls -lh "$OUTPUT_CHUNK" | awk '{print $5}')

                # Quick check if file has reasonable size (>1KB means it has points)
...
# List created chunks with point counts
